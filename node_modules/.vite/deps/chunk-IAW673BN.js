...
var require_with_selector_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      var React29 = require_react(), shim = require_shim(), objectIs = "function" === typeof Object.is ? Object.is : is, useSyncExternalStore2 = shim.useSyncExternalStore, useRef19 = React29.useRef, useEffect9 = React29.useEffect, useMemo17 = React29.useMemo, useDebugValue2 = React29.useDebugValue;
...
function useFocus(context, props = {}) {
  ...
  const blockedReferenceRef = React12.useRef(null);
...
function useHoverFloatingInteraction(context, parameters = {}) {
  ...
  const closeWithDelay = React13.useCallback((event) => {
...
function useHoverReferenceInteraction(context, props = {}) {
  ...
  const closeWithDelay = React14.useCallback((event, runElseBranch = true) => {
  ...
  React14.useEffect(() => cleanupMouseMoveHandler, [cleanupMouseMoveHandler]);
  const clearPointerEvents = useStableCallback(() => {
    clearSafePolygonPointerEventsMutation(instance);
  });
  React14.useEffect(() => {
...
function useInteractions(propsList = []) {
  ...
  const getTriggerProps = React15.useCallback(
...
function useListNavigation(context, props) {
  ...
  const focusItemOnOpenRef = React16.useRef(focusItemOnOpen);
  const indexRef = React16.useRef(selectedIndex ?? -1);
  const keyRef = React16.useRef(null);
  const isPointerModalityRef = React16.useRef(true);
  const onNavigate = useStableCallback((event) => {
    onNavigateProp(indexRef.current === -1 ? null : indexRef.current, event);
  });
  const previousOnNavigateRef = React16.useRef(onNavigate);
  const previousMountedRef = React16.useRef(!!floatingElement);
  const previousOpenRef = React16.useRef(open);
  const forceSyncFocusRef = React16.useRef(false);
  const forceScrollIntoViewRef = React16.useRef(false);
  ...
  const getParentOrientation = React16.useCallback(() => {
    var _a, _b, _c;
  ...
  const ariaActiveDescendantProp = React16.useMemo(() => {
...
function useRole(context, props = {}) {
  ...
  const floatingId = React17.useMemo(() => {
    var _a;
  ...
  const item = React17.useCallback(({
    active,
    selected
  }) => {
    const commonProps = {
...
function useTypeahead(context, props) {
  ...
  const prevIndexRef = React18.useRef(selectedIndex ?? activeIndex ?? -1);
...
var InternalBackdrop = React24.forwardRef(function InternalBackdrop2(props, ref) {
  const {
    cutout,
    ...otherProps
  } = props;
...
function useEnhancedClickHandler(handler) {
  const lastClickInteractionTypeRef = React26.useRef("");
  ...
  const handleClick = React26.useCallback((event) => {
    if (event.detail === 0) {
...
use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-IAW673BN.js.map
